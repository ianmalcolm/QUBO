...
class IFHOOS:
    ...
    def run(self):
        ...
        np.set_printoptions(threshold=np.inf)
        eighty_percent = math.ceil(self.size * 0.8)
        popular_items = self.popularity[0:eighty_percent]
        np.random.shuffle(popular_items)
        print("popularities: ", self.popularity)
    ...
    def sweep(self, ret, item_allocated, loc_allocated, popularity, distance):
        ...
        # print("after first round of sweep (assign to coi location): ")
        # print(items_unallocated)
        # print(locs_unallocated)
        # input()
        dtype_pop = [('idx',int),('pop',int)]
        ...
        np.random.shuffle(remainder_items)
        np.random.shuffle(remainder_locs)
    ...
    def allocate_pairs(self, ret, pairs, distance, item_allocated, loc_allocated, beta):
        ...
        for (j1,j2) in pairs2:
            loc_j1_final = item_allocated[j1]
            loc_j2_final = item_allocated[j2]
            if (not item_allocated[j1] is None) and item_allocated[j2] is None:
                locs_j2 = prepare_loc_set(j2)
                # print(locs_j2)
                # print("++++++++++++++++")
                # input()
