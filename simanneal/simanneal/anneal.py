...
class Annealer(object):
    ...
    def set_schedule(self, schedule):
        ...
        self.num_iter = schedule['num_iter']
    ...
    def anneal(self):
        ...
        coarse_steps = self.steps / self.num_iter
        coarse_step = 0
        while coarse_step < coarse_steps and not self.user_exit:
            print("coarse step %d" % coarse_step)
            for i in range(self.num_iter):
                ...
                if dE > 0.0 and math.exp(-dE / T) < random.random():
                    ...
                    if E < self.best_energy:
                        print("There is a new best energy %d" % E)
                        self.best_state = self.copy_state(self.state)
                        self.best_energy = E
                # if self.updates > 1:
                ...
                #         trials, accepts, improves = 0, 0, 0
            coarse_step += 1
            T = self.Tmax * math.exp(Tfactor * (coarse_step / coarse_steps))
    ...
    def auto(self, minutes, steps=1000000, num_iter=100):
